...
from mega import mega_to_text
from resources.lib import Menus
from resources.lib import utils
from resources.lib.Menus import getDataFromJson
from resources.lib.Menus import buildLibraryMenus_supreme
from resources.lib.Menus import searchItems
from resources.lib.Menus import listGenres
from resources.lib.Menus import listItemsByYear
from resources.lib.Menus import listYears
from resources.lib.Menus import listAllItems
from resources.lib.Menus import listItemsByGenre
from resources.lib.Menus import listRecentReleases
from resources.lib.Menus import parse_plugin_params
from resources.lib.library_series import getDataFromJsonSeries
from resources.lib.Menus import _log_debug, _log_error
...
if not xbmcvfs.exists(profile):
    try:
        ...
        pass
TOKEN_CODEBERG = '' # token de leitura codeberg.org private repository
MENU_URL = 'https://raw.githubusercontent.com/Animes13/OneX/refs/heads/main/menu.json'
...
def makeRequest(url, headers=None):
    try:
        ...
        if 'mega.nz' in url:
            try:
                return mega_to_text(url)  
            except Exception as e:
                addon_log("Erro ao processar mega.nz: {0}".format(str(e)))
                return ''
        elif 'codeberg.org' in url:
            headers['Authorization'] = 'token {0}'.format(TOKEN_CODEBERG)
            headers['User-Agent'] = 'Mozilla/5.0 (Linux; Android 13; SM-G991B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Mobile Safari/537.36'
            result = ''
            try:
                ...
                addon_log("Erro ao processar codeberg.org: {0}".format(str(e)))
                return ''
            if result:
                if result[-1] == '=':
                    try:
                        ...
                        addon_log("Erro ao decodificar base64: {0}".format(str(e)))
                        #return ''
            return result
...
def getData(url, fanart, data=None):
    xbmc.log('[ADDON-DEBUG] getData chamado - URL: %s' % url, xbmc.LOGINFO)
    xbmc.log('[ADDON-DEBUG] Iniciando detecção de JSON/XML', xbmc.LOGINFO)

    # --- Carregar conteúdo ---
    try:
        if data:
            soup = data
            xbmc.log('[ADDON-DEBUG] Usando objeto "data" recebido diretamente', xbmc.LOGINFO)
        else:
            soup = makeRequest(url)
            xbmc.log('[ADDON-DEBUG] Conteúdo carregado de %s' % url, xbmc.LOGINFO)
    except Exception as e_init:
        xbmc.log('[ADDON-ERROR] Erro ao carregar dados: %s' % str(e_init), xbmc.LOGERROR)
        return

    # --- JSON detectado ---
    try:
        if not isinstance(soup, ElementTree.Element):
            s_text = soup.decode('utf-8') if isinstance(soup, bytes) else (soup or '')
            s_strip = s_text.lstrip()

            if s_strip.startswith('{') or s_strip.startswith('['):
                try:
                    j = json.loads(s_text)

                    # 🚨 Agora em vez de chamar getDataFromJson direto,
                    # mandamos para getMainMenus decidir o caminho
                    xbmc.log('[ADDON-DEBUG] JSON detectado → repassando para getMainMenus', xbmc.LOGINFO)
                    from resources.lib.Menus import getMainMenus
                    return getMainMenus(j, fanart)

                except Exception as e_json:
                    xbmc.log('[ADDON-ERROR] Falha ao processar JSON no getData: %s' % str(e_json), xbmc.LOGERROR)
                    return
    except Exception as e_outer:
        xbmc.log('[ADDON-ERROR] Erro na detecção de JSON: %s' % str(e_outer), xbmc.LOGERROR)

    # --- XML detectado ---
    if isinstance(soup, ElementTree.Element):
        xbmc.log('[ADDON-DEBUG] XML detectado - raiz: %s' % getattr(soup, "tag", "??"), xbmc.LOGINFO)
        # fluxo XML igual como já está...
        channels = None
        if (
            (soup.tag == 'channels' and len(soup) > 0 and addon.getSetting('donotshowbychannels') == 'false')
            or (soup.tag == 'items' and len(soup) > 0)
        ):
            # processar canais normalmente...
            # ...
            pass
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits,
           showcontext=False, regexs=None, reg_url=None, allinfo={}):
    parentalblock = addon.getSetting('parentalblocked')
    parentalblock = parentalblock == "true"
    parentalblockedpin = addon.getSetting('parentalblockedpin')

    # 🚨 Não sobrescrever os modes principais
    if parentalblock:
        if name.lower() not in ["séries", "filmes", "documentários", "animes"]:
            mode = 58

    # Debug para saber o que está sendo passado
    xbmc.log(f"[ADDON-DEBUG] addDir: name={name} | url={url} | mode={mode}", xbmc.LOGINFO)

    # Evita bug de paths .xml
    url = url + "/" if url.endswith(".xml") else url

    if regexs and len(regexs) > 0:
        u = (sys.argv[0] + "?url=" + urllib_parse.quote_plus(url) +
             "&mode=" + str(mode) +
             "&name=" + urllib_parse.quote_plus(name) +
             "&fanart=" + urllib_parse.quote_plus(fanart) +
             "&regexs=" + regexs)
    else:
        u = (sys.argv[0] + "?url=" + urllib_parse.quote_plus(url) +
             "&mode=" + str(mode) +
    ...
    if len(allinfo) < 1:
        if get_kversion() > 19:
            ...
            liz.setInfo(type="Video", infoLabels={
                "Title": name, 'mediatype': 'video', "Plot": description,
                "Genre": genre, "dateadded": date, "credits": credits
            })
    else:
        if get_kversion() > 19:
            info = liz.getVideoInfoTag()
    ...
    if showcontext:
        ...
        if len(parentalblockedpin) > 0:
            if parentalblock:
                contextMenu.append(('Disable Parental Block',
                                    'RunPlugin(%s?mode=55&name=%s)' %
                                    (sys.argv[0], urllib_parse.quote_plus(name))))
            else:
                contextMenu.append(('Enable Parental Block',
                                    'RunPlugin(%s?mode=56&name=%s)' %
                                    (sys.argv[0], urllib_parse.quote_plus(name))))
        if showcontext == 'source':
            if name in str(SOURCES):
                contextMenu.append(('Remove from Sources',
                                    'RunPlugin(%s?mode=8&name=%s)' %
                                    (sys.argv[0], urllib_parse.quote_plus(name))))
        elif showcontext == 'download':
            contextMenu.append(('Download',
                                'RunPlugin(%s?url=%s&mode=9&name=%s)' %
                                (sys.argv[0], urllib_parse.quote_plus(url),
                                 urllib_parse.quote_plus(name))))
        elif showcontext == 'fav':
            contextMenu.append(('Remove from %s Favorites' % str(name_addon_info),
                                'RunPlugin(%s?mode=6&name=%s)' %
                                (sys.argv[0], urllib_parse.quote_plus(name))))
        if showcontext == '!!update':
            fav_params2 = ('%s?url=%s&mode=17&regexs=%s' %
                           (sys.argv[0], urllib_parse.quote_plus(reg_url), regexs))
            contextMenu.append(('[COLOR yellow]!!update[/COLOR]', 'RunPlugin(%s)' % fav_params2))
        if name not in FAV:
            contextMenu.append(('Add to %s Favorites' % str(name_addon_info),
                                'RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)' %
                                (sys.argv[0], urllib_parse.quote_plus(name),
                                 urllib_parse.quote_plus(url),
                                 urllib_parse.quote_plus(iconimage),
                                 urllib_parse.quote_plus(fanart), mode)))
...
elif mode == 99:  # submenus
    try:
        import json as _json
        submenus = _json.loads(url)  # o url aqui vem como payload JSON
        getMainMenus(submenus, FANART)
    except Exception as e:
        xbmc.log("[ADDON-ERROR] Falha ao carregar submenus: %s" % str(e), xbmc.LOGERROR)
        xbmcgui.Dialog().notification(addon_name, "Erro ao abrir submenus", icon, 4000, False)


elif mode == 100:  # 🎬 Filmes → baixa o JSON, cria cache e redireciona
    _log_debug("[SUPREME][FLOW] Entrando no modo 100 (Filmes)")

    try:
        # 🔹 Lê parâmetros de forma segura
        from resources.lib.Menus import parse_plugin_params
        params = parse_plugin_params()
        raw_url = params.get("url", "")
        fanart = params.get("fanart", "")

        _log_debug(f"[SUPREME][FLOW] Modo 100 — raw_url len={len(str(raw_url))} preview={str(raw_url)[:100]}")

        # 🔹 Caso 1 — Payload já é JSON embutido
        if isinstance(raw_url, str) and (raw_url.strip().startswith("[") or raw_url.strip().startswith("{")):
            _log_debug("[SUPREME][FLOW] Payload detectado inline (JSON embutido) → enviando para getDataFromJson")
            from resources.lib.Menus import getDataFromJson
            getDataFromJson(raw_url, fanart)
            xbmcplugin.endOfDirectory(int(sys.argv[1]))
            _log_debug("[SUPREME][FLOW] Modo 100 finalizado (JSON inline)")
        
        # 🔹 Caso 2 — URL remota (HTTP/HTTPS)
        elif isinstance(raw_url, str) and raw_url.startswith("http"):
            _log_debug(f"[SUPREME][FLOW] Fazendo requisição remota: {raw_url}")
            from resources.lib.utils import makeRequest
            data = makeRequest(raw_url)

            if data:
                _log_debug(f"[SUPREME][FLOW] Resposta recebida (len={len(data)}) → enviando para getDataFromJson")
                from resources.lib.Menus import getDataFromJson
                getDataFromJson(data, fanart)
                xbmcplugin.endOfDirectory(int(sys.argv[1]))
                _log_debug("[SUPREME][FLOW] Modo 100 finalizado (requisição remota OK)")
            else:
                _log_error("[SUPREME][FLOW] Resposta vazia da URL — abortando")
                xbmcgui.Dialog().notification("Erro", "Biblioteca vazia ou inacessível", xbmcgui.NOTIFICATION_ERROR, 4000)
                xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

        # 🔹 Caso 3 — Payload codificado (Base64 ou %7B)
        else:
            _log_debug("[SUPREME][FLOW] Tentando decodificação de payload codificado")
            import urllib.parse, base64

            decoded = None
            try:
                decoded = urllib.parse.unquote_plus(raw_url)
                _log_debug(f"[SUPREME][FLOW] unquote_plus OK (len={len(decoded)}) preview={decoded[:80]}")
            except Exception as e_up:
                _log_error(f"[SUPREME][FLOW] unquote_plus falhou: {e_up}")

            if not decoded:
                try:
                    pad = '=' * ((4 - len(raw_url) % 4) % 4)
                    decoded = base64.urlsafe_b64decode(raw_url + pad).decode("utf-8")
                    _log_debug(f"[SUPREME][FLOW] Base64 OK (len={len(decoded)}) preview={decoded[:80]}")
                except Exception as e_b64:
                    _log_error(f"[SUPREME][FLOW] Falha ao decodificar Base64: {e_b64}")

            if decoded and (decoded.strip().startswith("[") or decoded.strip().startswith("{")):
                _log_debug("[SUPREME][FLOW] Payload decodificado com sucesso → enviando para getDataFromJson")
                from resources.lib.Menus import getDataFromJson
                getDataFromJson(decoded, fanart)
                xbmcplugin.endOfDirectory(int(sys.argv[1]))
                _log_debug("[SUPREME][FLOW] Modo 100 finalizado (payload decodificado OK)")
            else:
                _log_error(f"[SUPREME][FLOW] Nenhum payload válido encontrado em mode=100 (url preview={raw_url[:100]})")
                xbmcgui.Dialog().notification("Erro", "Nenhum payload válido", xbmcgui.NOTIFICATION_ERROR, 4000)
                xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

    except Exception as e:
        import traceback
        _log_error(f"[SUPREME][FLOW] Erro fatal no modo 100: {e}")
        _log_error(traceback.format_exc())
        xbmcgui.Dialog().notification("Erro", "Falha ao processar biblioteca de filmes", xbmcgui.NOTIFICATION_ERROR, 4000)
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)


elif mode == 101:  # 🎬 Todos os Filmes
    from resources.lib.Menus import listAllItems
    listAllItems(fanart)

elif mode == 102:  # 🔍 Pesquisar
    keyboard = xbmc.Keyboard('', 'Buscar Filme')
    keyboard.doModal()
    if keyboard.isConfirmed():
        query = keyboard.getText()
        if query:
            from resources.lib.Menus import searchItems
            searchItems(query, fanart)
        else:
            xbmcgui.Dialog().notification("Busca", "Nenhum termo digitado", xbmcgui.NOTIFICATION_INFO, 3000)
            
elif mode == 103:
    from resources.lib.Menus import listGenres
    payload = params.get("url", "")
    listGenres(payload, fanart)

elif mode == 112:
    from resources.lib.Menus import listItemsByGenre
    payload = params.get("url", "")
    genre = params.get("genre", "")
    listItemsByGenre(payload, fanart, genre)

elif mode == 108:
    # 📅 "Por Ano" → mostra os anos disponíveis
    _log_debug("[FLOW] Entrando em modo 108 → listYears")
    try:
        params = parse_plugin_params()
        payload = params.get("url", "")
        fanart = params.get("fanart", "")
        _log_debug(f"[FLOW] listYears chamado (len payload={len(str(payload))})")
        listYears(payload, fanart)
    except Exception as e:
        import traceback
        _log_error(f"[FLOW] Erro ao executar listYears: {e}")
        _log_error(traceback.format_exc())
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

elif mode == 109:
    # 🎬 Itens filtrados por ano específico
    _log_debug("[FLOW] Entrando em modo 109 → listItemsByYear")
    try:
        params = parse_plugin_params()
        payload = params.get("url", "")
        fanart = params.get("fanart", "")
        ano = params.get("name", "")
        _log_debug(f"[FLOW] listItemsByYear chamado (ano={ano}, len payload={len(str(payload))})")
        listItemsByYear(payload, fanart, ano)
    except Exception as e:
        import traceback
        _log_error(f"[FLOW] Erro ao executar listItemsByYear: {e}")
        _log_error(traceback.format_exc())
        xbmcplugin.endOfDirectory(int(sys.argv[1]), succeeded=False)

elif mode == 110:  # 🏭 Por Estúdio
    from resources.lib.Menus import listStudios
    listStudios(None, fanart)

elif mode == 111:  # Filmes filtrados por Estúdio
    try:
        studio = params.get("studio", [""])[0]
        from resources.lib.Menus import listItemsByStudio
        listItemsByStudio(studio, fanart)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR][FILMES] Erro ao abrir estúdio: {e}", xbmc.LOGERROR)

elif mode == 200:  # Menu inicial de Séries
    try:
        from resources.lib import library_series
        import json as _json

        xbmc.log("[ADDON-DEBUG] Entrou no mode 200 (Séries)", xbmc.LOGINFO)

        # Se a URL for um JSON remoto → baixar
        series_data = None
        if url and url.startswith("http"):
            xbmc.log(f"[ADDON-DEBUG] Baixando JSON de séries: {url}", xbmc.LOGINFO)
            soup = makeRequest(url)
            j = _json.loads(soup)
            series_data = j.get("series") if isinstance(j, dict) else j
        else:
            # Se já veio payload direto
            try:
                series_data = _json.loads(url)
            except Exception:
                series_data = url

        xbmc.log(f"[ADDON-DEBUG] Dados de séries carregados: {type(series_data)} | "
                 f"Qtd={len(series_data) if isinstance(series_data, list) else 'N/A'}", xbmc.LOGINFO)

        if hasattr(library_series, "buildSeriesLibraryMenus"):
            library_series.buildSeriesLibraryMenus(series_data, FANART)
        else:
            xbmc.log("[ADDON-ERROR] buildSeriesLibraryMenus não encontrado em library_series", xbmc.LOGERROR)

    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] Erro no mode 200 (Séries): {e}", xbmc.LOGERROR)


elif mode == 201:  # Tela de Temporadas de uma série
    try:
        from resources.lib import library_series
        tv_id = params.get("tv_id")
        payload = params.get("payload")
        if not tv_id:
            raise Exception("tv_id não informado")
        library_series.openSeries(int(tv_id), payload, FANART)
    except Exception as e:
        xbmc.log("[ADDON-ERROR] openSeries falhou: %s" % str(e), xbmc.LOGERROR)


elif mode == 202:  # Tela de Episódios de uma temporada
    try:
        from resources.lib import library_series
        tv_id = params.get("tv_id")
        ...
        if not tv_id or not season:
            raise Exception("tv_id ou season não informado")
        library_series.openSeason(int(tv_id), int(season), payload, FANART)
    except Exception as e:
        xbmc.log("[ADDON-ERROR] openSeason falhou: %s" % str(e), xbmc.LOGERROR)


elif mode == 203:  # Reprodução de episódio
    try:
        from resources.lib import library_series
        tv_id = params.get("tv_id")
        season = params.get("season")
        episode = params.get("episode")
        url = params.get("play_url")
        if not (tv_id and season and episode and url):
            raise Exception("Parâmetros incompletos para reprodução")
        library_series.playEpisode(int(tv_id), int(season), int(episode), url)
    except Exception as e:
        xbmc.log("[ADDON-ERROR] playEpisode falhou: %s" % str(e), xbmc.LOGERROR)
        
        
elif mode == 213:  # Todas as Séries
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listAllSeries(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listAllSeries falhou: {e}", xbmc.LOGERROR)

elif mode == 208:  # Pesquisar
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.searchSeries(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] searchSeries falhou: {e}", xbmc.LOGERROR)

elif mode == 206:  # Lista de Gêneros
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listSeriesGenres(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesGenres falhou: {e}", xbmc.LOGERROR)

elif mode == 207:  # Séries por Gênero
    try:
        from resources.lib import library_series
        payload = params.get("url")
        genre = params.get("genre")
        library_series.listSeriesByGenre(payload, FANART, genre)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesByGenre falhou: {e}", xbmc.LOGERROR)

elif mode == 209:  # Lista de Anos
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listSeriesYears(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesYears falhou: {e}", xbmc.LOGERROR)

elif mode == 210:  # Séries por Ano
    try:
        from resources.lib import library_series
        payload = params.get("url")
        year = params.get("year")
        library_series.listSeriesByYear(payload, FANART, year)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesByYear falhou: {e}", xbmc.LOGERROR)

elif mode == 211:  # Lista de Estúdios
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listSeriesStudios(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesStudios falhou: {e}", xbmc.LOGERROR)

elif mode == 212:  # Séries por Estúdio
    try:
        from resources.lib import library_series
        payload = params.get("url")
        studio = params.get("studio")
        library_series.listSeriesByStudio(payload, FANART, studio)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listSeriesByStudio falhou: {e}", xbmc.LOGERROR)

elif mode == 214:  # Lançamentos (último mês)
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listRecentSeries(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listRecentSeries falhou: {e}", xbmc.LOGERROR)

elif mode == 215:  # Novas Temporadas (últimos 30 dias)
    try:
        from resources.lib import library_series
        payload = params.get("url")
        library_series.listRecentSeasons(payload, FANART)
    except Exception as e:
        xbmc.log(f"[ADDON-ERROR] listRecentSeasons falhou: {e}", xbmc.LOGERROR)
